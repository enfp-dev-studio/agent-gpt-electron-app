...
class AutonomousAgent {
  ...
  private maxLoops() {
    // Only handle custom max loops for custom API key
    return this.modelSettings.customMaxLoops
      ? this.modelSettings.customMaxLoops
  ...
  private shouldRunClientSide() {
    // Agent GPT App will run client side only
    return true
  }
  ...
  sendLoopMessage() {
    this.sendMessage({
      type: 'system',
      value: `This agent has maxed out on loops. To save your wallet, this agent is shutting down. You can configure the number of loops in the advanced settings.`
